...
// 过期提醒
#define IRDeprecated(instead) NS_DEPRECATED(2_0, 2_0, 2_0, 2_0, instead)

typedef NS_ENUM(NSInteger , IRPopupMenuType) {
    IRPopupMenuTypeDefault = 0,
    IRPopupMenuTypeDark
};
...
/**
 ...
 当控件超出屏幕时会自动调整成反方向
 */
typedef NS_ENUM(NSInteger , IRPopupMenuPriorityDirection) {
    IRPopupMenuPriorityDirectionTop = 0,  //Default
    IRPopupMenuPriorityDirectionBottom,
    IRPopupMenuPriorityDirectionLeft,
    IRPopupMenuPriorityDirectionRight,
    IRPopupMenuPriorityDirectionNone      //不自动调整
};

@class IRPopupMenu;
@protocol IRPopupMenuDelegate <NSObject>
...
/**
 点击事件回调
 */
- (void)irPopupMenuDidSelectedAtIndex:(NSInteger)index irPopupMenu:(IRPopupMenu *)irPopupMenu IRDeprecated("请替用 irPopupMenu: didSelectedAtIndex: 方法");
- (void)irPopupMenuBeganDismiss;
- (void)irPopupMenuDidDismiss;
- (void)irPopupMenuBeganShow;
- (void)irPopupMenuDidShow;
...
/**
 ...
 只有箭头优先级是IRPopupMenuPriorityDirectionLeft/IRPopupMenuPriorityDirectionRight/IRPopupMenuPriorityDirectionNone时需要设置
 */
@property (nonatomic, assign) CGFloat arrowPosition;

/**
 箭头方向 Default is IRPopupMenuArrowDirectionTop
 */
@property (nonatomic, assign) IRPopupMenuArrowDirection arrowDirection;

/**
 箭头优先方向 Default is IRPopupMenuPriorityDirectionTop
 
 当控件超出屏幕时会自动调整箭头位置
 */
@property (nonatomic, assign) IRPopupMenuPriorityDirection priorityDirection;
...
/**
 设置显示模式 自定义cell时忽略 Default is IRPopupMenuTypeDefault
 */
@property (nonatomic, assign) IRPopupMenuType type;
...
/**
 代理
 */
@property (nonatomic, weak) id <IRPopupMenuDelegate> delegate;
...
/**
 ...
 @param delegate  代理
 */
+ (IRPopupMenu *)showAtPoint:(CGPoint)point
                      titles:(NSArray *)titles
                       icons:(NSArray *)icons
                   menuWidth:(CGFloat)itemWidth
                    delegate:(id<IRPopupMenuDelegate>)delegate;
...
+ (IRPopupMenu *)showAtPoint:(CGPoint)point
                      titles:(NSArray *)titles
                       icons:(NSArray *)icons
                   menuWidth:(CGFloat)itemWidth
               otherSettings:(void (^) (IRPopupMenu * popupMenu))otherSetting;
...
/**
 ...
 @param delegate  代理
 */
+ (IRPopupMenu *)showRelyOnView:(UIView *)view
                         titles:(NSArray *)titles
                          icons:(NSArray *)icons
                      menuWidth:(CGFloat)itemWidth
                       delegate:(id<IRPopupMenuDelegate>)delegate;
