...
#import "IRPopupMenuImp.h"
#import "IRPopupMenuPath.h"

#define IRScreenWidth [UIScreen mainScreen].bounds.size.width
#define IRScreenHeight [UIScreen mainScreen].bounds.size.height
#define IRMainWindow  [UIApplication sharedApplication].keyWindow
#define IR_SAFE_BLOCK(BlockName, ...) ({ !BlockName ? nil : BlockName(__VA_ARGS__); })

#pragma mark - /////////////
#pragma mark - private cell

@interface IRPopupMenuCell : UITableViewCell
@property (nonatomic, assign) BOOL isShowSeparator;
@property (nonatomic, strong) UIColor * separatorColor;
@end

@implementation IRPopupMenuCell
...
#pragma mark - publics
+ (IRPopupMenu *)showAtPoint:(CGPoint)point titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth delegate:(id<IRPopupMenuDelegate>)delegate
{
    IRPopupMenu *popupMenu = [[IRPopupMenu alloc] init];
...
+ (IRPopupMenu *)showRelyOnView:(UIView *)view titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth delegate:(id<IRPopupMenuDelegate>)delegate
{
    CGRect absoluteRect = [view convertRect:view.bounds toView:IRMainWindow];
...
+ (IRPopupMenu *)showAtPoint:(CGPoint)point titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth otherSettings:(void (^) (IRPopupMenu * popupMenu))otherSetting
{
    IRPopupMenu *popupMenu = [[IRPopupMenu alloc] init];
    popupMenu.point = point;
    popupMenu.titles = titles;
    popupMenu.images = icons;
    popupMenu.itemWidth = itemWidth;
    IR_SAFE_BLOCK(otherSetting,popupMenu);
    [popupMenu show];
    return popupMenu;
}

+ (IRPopupMenu *)showRelyOnView:(UIView *)view titles:(NSArray *)titles icons:(NSArray *)icons menuWidth:(CGFloat)itemWidth otherSettings:(void (^) (IRPopupMenu * popupMenu))otherSetting
{
    CGRect absoluteRect = [view convertRect:view.bounds toView:IRMainWindow];
    CGPoint relyPoint = CGPointMake(absoluteRect.origin.x + absoluteRect.size.width / 2, absoluteRect.origin.y + absoluteRect.size.height);
    IRPopupMenu *popupMenu = [[IRPopupMenu alloc] init];
...
- (void)dismiss
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(irPopupMenuBeganDismiss)]) {
        [self.delegate irPopupMenuBeganDismiss];
    }
    [UIView animateWithDuration: 0.25 animations:^{
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(irPopupMenuDidDismiss)]) {
            [self.delegate irPopupMenuDidDismiss];
        }
        self.delegate = nil;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(irPopupMenu:cellForRowAtIndex:)]) {
        tableViewCell = [self.delegate irPopupMenu:self cellForRowAtIndex:indexPath.row];
    }
    ...
    static NSString * identifier = @"irPopupMenu";
    IRPopupMenuCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (!cell) {
        cell = [[IRPopupMenuCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:identifier];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(irPopupMenuDidSelectedAtIndex:irPopupMenu:)]) {
        
        [self.delegate irPopupMenuDidSelectedAtIndex:indexPath.row irPopupMenu:self];
    }
    
    if (self.delegate && [self.delegate respondsToSelector:@selector(irPopupMenu:didSelectedAtIndex:)]) {
        [self.delegate irPopupMenu:self didSelectedAtIndex:indexPath.row];
    }
}
...
{
    if ([[self getLastVisibleCell] isKindOfClass:[IRPopupMenuCell class]]) {
        IRPopupMenuCell *cell = [self getLastVisibleCell];
...
- (IRPopupMenuCell *)getLastVisibleCell
{
...
- (void)show
{
    [IRMainWindow addSubview:_menuBackView];
    [IRMainWindow addSubview:self];
    if ([[self getLastVisibleCell] isKindOfClass:[IRPopupMenuCell class]]) {
        IRPopupMenuCell *cell = [self getLastVisibleCell];
        cell.isShowSeparator = NO;
    }
    if (self.delegate && [self.delegate respondsToSelector:@selector(irPopupMenuBeganShow)]) {
        [self.delegate irPopupMenuBeganShow];
    }
    self.layer.affineTransform = CGAffineTransformMakeScale(0.1, 0.1);
    [UIView animateWithDuration: 0.25 animations:^{
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(irPopupMenuDidShow)]) {
            [self.delegate irPopupMenuDidShow];
        }
    }];
}
...
- (void)setDefaultSettings
{
    ...
    _type = IRPopupMenuTypeDefault;
    _arrowDirection = IRPopupMenuArrowDirectionTop;
    _priorityDirection = IRPopupMenuPriorityDirectionTop;
    _minSpace = 10.0;
    _maxVisibleCount = 5;
    _itemHeight = 44;
    _isCornerChanged = NO;
    _showMaskView = YES;
    _menuBackView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, IRScreenWidth, IRScreenHeight)];
...
- (void)setType:(IRPopupMenuType)type
{
    _type = type;
    switch (type) {
...
- (void)setArrowDirection:(IRPopupMenuArrowDirection)arrowDirection
{
...
- (void)setPriorityDirection:(IRPopupMenuPriorityDirection)priorityDirection
{
...
- (void)updateUI
{
    ...
    if (_priorityDirection == IRPopupMenuPriorityDirectionTop) {
        if (_point.y + height + _arrowHeight > IRScreenHeight - _minSpace) {
    ...
    }else if (_priorityDirection == IRPopupMenuPriorityDirectionBottom) {
    ...
    }else if (_priorityDirection == IRPopupMenuPriorityDirectionLeft) {
        if (_point.x + _itemWidth + _arrowHeight > IRScreenWidth - _minSpace) {
    ...
    }else if (_priorityDirection == IRPopupMenuPriorityDirectionRight) {
    ...
    [self setRelyRect];
    if (_arrowDirection == IRPopupMenuArrowDirectionTop) {
        CGFloat y = _isChangeDirection ? _point.y  : _point.y;
        if (_arrowPosition > _itemWidth / 2) {
            self.frame = CGRectMake(IRScreenWidth - _minSpace - _itemWidth, y, _itemWidth, height + _arrowHeight);
    ...
    }else if (_arrowDirection == IRPopupMenuArrowDirectionBottom) {
    ...
    }else if (_arrowDirection == IRPopupMenuArrowDirectionLeft) {
    ...
    }else if (_arrowDirection == IRPopupMenuArrowDirectionRight) {
    ...
    }else if(_arrowDirection == IRPopupMenuArrowDirectionCircle) {
        CGFloat y = _isChangeDirection ? _point.y  : _point.y;
        self.frame = CGRectMake(_point.x, y, _itemWidth, height + _arrowHeight);
    }else if (_arrowDirection == IRPopupMenuArrowDirectionNone) {
        CGFloat y = _isChangeDirection ? _point.y  : _point.y;
        self.frame = CGRectMake(_point.x, y, _itemWidth, height);
    }
    ...
    UIBezierPath *path = [UIBezierPath bezierPathWithRoundedRect:self.tableView.bounds byRoundingCorners:_rectCorner cornerRadii:CGSizeMake(_cornerRadius, _cornerRadius)];
...
- (void)changeRectCorner
{
    ...
    if (_arrowDirection == IRPopupMenuArrowDirectionTop || _arrowDirection == IRPopupMenuArrowDirectionBottom) {
    ...
    }else if (_arrowDirection == IRPopupMenuArrowDirectionLeft || _arrowDirection == IRPopupMenuArrowDirectionRight) {
        if (haveTopLeftCorner) {
            _rectCorner = _rectCorner | UIRectCornerTopRight;
        }else {
            _rectCorner = _rectCorner & (~UIRectCornerTopRight);
...
{
    if (_priorityDirection == IRPopupMenuPriorityDirectionNone) {
        return;
    }
    if (_arrowDirection == IRPopupMenuArrowDirectionTop || _arrowDirection == IRPopupMenuArrowDirectionBottom) {
        if (_point.x + _itemWidth / 2 > IRScreenWidth - _minSpace) {
            _arrowPosition = _itemWidth - (IRScreenWidth - _minSpace - _point.x);
...
- (void)drawRect:(CGRect)rect
{
    UIBezierPath *bezierPath = [IRPopupMenuPath ir_bezierPathWithRect:rect rectCorner:_rectCorner cornerRadius:_cornerRadius borderWidth:_borderWidth borderColor:_borderColor backgroundColor:_backColor arrowWidth:_arrowWidth arrowHeight:_arrowHeight arrowPosition:_arrowPosition arrowDirection:_arrowDirection];
    [bezierPath fill];
    [bezierPath stroke];
}
